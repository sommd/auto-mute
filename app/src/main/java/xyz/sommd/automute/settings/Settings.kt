...
class Settings(private val context: Context): SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    var serviceEnabled: Boolean
        get() = sharedPrefs.getBoolean(
                SERVICE_ENABLED_KEY, true)
        set(value) = sharedPrefs.edit { putBoolean(
                SERVICE_ENABLED_KEY, value) }
    
    var autoMuteEnabled: Boolean
        get() = sharedPrefs.getBoolean(
                AUTO_MUTE_ENABLED_KEY, true)
        set(value) = sharedPrefs.edit { putBoolean(
                AUTO_MUTE_ENABLED_KEY, value) }
    
    // TODO use int pref
    var autoMuteDelay: Int
        get() = sharedPrefs.getString(
                AUTO_MUTE_DELAY_KEY, "30").toInt()
        set(value) = sharedPrefs.edit { putString(
                AUTO_MUTE_DELAY_KEY, value.toString()) }
    
    var autoMuteToast: Boolean
        get() = sharedPrefs.getBoolean(
                AUTO_MUTE_TOAST_KEY, false)
        set(value) = sharedPrefs.edit { putBoolean(
                AUTO_MUTE_TOAST_KEY, value) }
    
    var autoUnmuteDefaultVolume: Int
        get() = sharedPrefs.getInt(
                AUTO_UNMUTE_DEFAULT_VOLUME_KEY, 50)
        set(value) = sharedPrefs.edit { putInt(
                AUTO_UNMUTE_DEFAULT_VOLUME_KEY, value) }
    
    var autoUnmuteMusicMode: UnmuteMode
        get() = UnmuteMode.valueOf(sharedPrefs.getString(
                AUTO_UNMUTE_MUSIC_MODE_KEY, "ALWAYS"))
        set(value) = sharedPrefs.edit { putString(
                AUTO_UNMUTE_MUSIC_MODE_KEY, value.name) }
    
    var autoUnmuteMediaMode: UnmuteMode
        get() = UnmuteMode.valueOf(sharedPrefs.getString(
                AUTO_UNMUTE_MEDIA_MODE_KEY, "ASK"))
        set(value) = sharedPrefs.edit { putString(
                AUTO_UNMUTE_MEDIA_MODE_KEY, value.name) }
    
    var autoUnmuteAssistantMode: UnmuteMode
        get() = UnmuteMode.valueOf(sharedPrefs.getString(
                AUTO_UNMUTE_ASSISTANT_MODE_KEY, "ALWAYS"))
        set(value) = sharedPrefs.edit { putString(
                AUTO_UNMUTE_ASSISTANT_MODE_KEY, value.name) }
